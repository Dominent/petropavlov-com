...
{/* ISR — regenerate every 60s so the inlined experiments JSON
    (rendered by app/_components/experiments-script.tsx inside
    app/layout.tsx) refreshes between deploys. JSX-style comment
    here because MDX treats loose `//` lines as markdown paragraphs
    and they bleed into the rendered HTML. */}
export const revalidate = 60

{/* Tight 152-char description — Google truncates SERP snippets at
    ~160 chars. The previous 249-char version was getting cut after
    "I should not run it yet", losing the framework + spreadsheet
    pitch. Exported (not bare const) because MDX only recognises
    import/export at the file's module scope. */}
export const DESCRIPTION =
  "I built A/B testing into my portfolio. Then did the math and realised I shouldn't run it yet. The framework, the spreadsheet, and when to actually ship."
...
export const metadata = {
  ...
  // Override the layout-level keywords (which list company names +
  // locations from the home-page targeting) with post-topic terms.
  keywords: [
    'A/B testing',
    'feature flags',
...
export const ARTICLE_SCHEMA = {
  ...
  // Full ISO 8601 with timezone — Google prefers timezone-precise
  // dates over date-only strings; gives a cleaner freshness signal.
  ...
  // wordCount tells crawlers this is long-form content; AI search
  // overviews (Perplexity, ChatGPT browse) weight depth signals when
  // ranking source articles for synthesis.
  wordCount: 2092,
  // articleSection — broad topical category, useful for topical
  // clustering and "more in Engineering" related-content widgets.
...
My [portfolio](/) gets about 20 visits a day. 77% of those visitors leave from the hero before scrolling to [the work section](/#work). Last week I built a full A/B testing framework into it — feature flags, sticky variant assignment, an admin dashboard with Wilson confidence intervals and two-proportion z-tests, the works.
...
The whole thing — Pulse, the experiment framework, this blog, the Next.js migration that made it all coherent — is open in spirit on [petropavlov.dev](/). If you read any of the stack and have notes, I'd genuinely like to hear them. Email's at the bottom of every page. There are two deeper case studies under the same roof — the [AI meeting SaaS I co-founded](/case-studies/insight-draft) and the [EU Digital Identity Wallet SDK](/case-studies/gramota) — both with the same flavour of honest writeup as this one.
